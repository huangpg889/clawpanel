...
export async function render() {
  ...
  page.innerHTML = `
    ...
    <div id="qtcool-promo" style="display:none;margin-bottom:var(--space-lg);border-radius:12px;background:linear-gradient(135deg,#0f0c29 0%,#302b63 50%,#24243e 100%);color:#fff;position:relative;overflow:hidden;box-shadow:0 4px 24px rgba(48,43,99,0.25)">
...
async function fetchRemoteModels(btn, page, state, providerKey) {
  ...
  if (!provider?.baseUrl) {
    toast('请先在「编辑」中填写服务商的接口地址（baseUrl）', 'warning')
    return
  }

  // 如果 apiKey 是 SecretRef 对象，传空字符串（Rust 侧无法解析对象为字符串）
  ...
  try {
    const remoteIds = await api.listRemoteModels(provider.baseUrl, apiKey, provider.api || 'openai-completions')
    btn.disabled = false
    ...
    btn.disabled = false
    btn.textContent = '获取列表'
    toast(`获取模型列表失败: ${typeof e === 'string' ? e : e?.message || String(e)}`, 'error')
  }
}
